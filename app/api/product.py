...
from fastapi import Depends, APIRouter, Form, UploadFile, File, Path, HTTPException
...
@router.get("/product/{id}/embedding",
            response_model=ApiResponse[ProductEmbeddingResponse],
            dependencies=[Depends(require_admin)])
def get_product_by_id(
        db: Session = Depends(get_db),
        id: UUID = Path(...)
    ...
    products = ProductService.get_product_embedding_by_id(db, id)
    if not products:
        raise HTTPException(
            status_code=404,
    ...
    return {
        "status": 200,
        "message": "Get Products Embedding Successfully",
        "data": products
    }
...
@router.patch("/products/{product_id}/embedding")
def update_product_embedding_status(
        product_id: UUID = Path(...),
        db: Session = Depends(get_db)
):
    products = ProductService.create_embedding_for_product(db, product_id)
    return {
        "status": 200,
